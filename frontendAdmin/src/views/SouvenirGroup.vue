...
import { HTTP } from "@/Connect/httpClient";
  export default {
    ...
    methods: {
      async initialize() {   
            try { 
                HTTP.get('souvenirgroup').then((res) => {   this.desserts = res.data.data   }) 
            } catch (error) {
                console.error(error)
            }
        },    
      ...
      deleteItem (item,id) {
        ...
        HTTP.delete('souvenirgroup/'+id).then((res) => { console.log(res.data.data)  }) 
      },
      ...
      save () {
        if (this.editedIndex > -1) { 
          ...
          HTTP.post('souvenirgroup/'+this.id,obj).then((res) => {   
            console.log(res)
            this.initialize() 
          })  
        } else { 
          let obj = this.editedItem
          HTTP.post('souvenirgroup',obj).then((res) => {   
            console.log(res)
            this.initialize()    
          }) 
