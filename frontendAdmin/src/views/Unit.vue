...
components: {
    ...
    methods: {
      async initialize() {   
            try { 
                HTTP.get('unit').then((res) => {   this.desserts = res.data.data   }) 
            } catch (error) {
                console.error(error)
            }
        },    
      ...
      deleteItem (item,id) {
        ...
        HTTP.delete('unit/'+id).then((res) => { console.log(res.data.data)  }) 
      },
      ...
      save () {
        if (this.editedIndex > -1) { 
          ...
          HTTP.post('unit/'+this.id,obj).then((res) => {   
            console.log(res)
            this.initialize()
           }) 
